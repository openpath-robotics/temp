...
class PHOXI_DLL_API PhoXi : public PhoXiInterface {
  ...
  public:
    ...
    PhoXiDeviceInformation Info() const;
    ...
    */
    ...
    ///Triggers the Frame.
    /**
    Triggers the Frame (if in Software Trigger mode) and returns FrameID on
    success. The FrameID can be used to pair the Frames received by GetFrame
    with the Software Triggers. If the device is not in Software trigger mode,
    the call will return negative number.
    If WaitForAccept is true, the call will wait until the device will be
    ready to accept the Software trigger.
    If WaitForAccept if false, the call will return with negative number after
    the unsuccessful Software trigger attempt (i.e. multiple Rapid Software
    triggers).
    If WaitForGrabbingEnd is true, the call will wait until the end of Device
    Grabbing and then returns (useful for Data Grabbing Sync between more
    devices).
    @param WaitForAccept - Wait for the device until ready for Software Trigger
    @param CustomMessage - Custom string that will be transfered and attached
        to the frame (can be used to transfer robotic coordinates, etc.).
        Requirements: PhoXiControl-1.2.x (or newer).
    @return positive FrameID on success, negative number on failure:
        ...
        -100 and less Internal error code
    */
    ...
    preferred (e.g. C:/Users/Public/my.praw, /home/user/my.praw), relative path
    points to PhoXiControl's path (folder should exist).
    @param FrameID - Save only if last output scan has the same frame index.
    @param jsonOptions - (optional) Configure save operations with options
    @return true on success, false otherwise.
    ...
    */
    virtual bool SaveLastOutput(const std::string &FilePath, int FrameID = -1, const std::string& jsonOptions = "") override;
...
#ifdef PHOXI_API_CLIENT
    ///Starts recording of frames
    /**
    Starts recording of triggered or free-run frames to formats specified in the options.
    @param jsonOptions - JSON string containing recording options. JSON object can be partial, 
    non present values will be defaulted to `false` or to default value specified in the schema. 
    The JSON options are described by schema RecordingOptionsSchema.json or 
    RecordingOptionsSchemaStrict.json (with all fields as required).
    @param persistOptions - Persist options for this device. Options will be loaded and applied
    when device is connected.
    @return StartRecordingResult
    */
    virtual StartRecordingResult StartRecording(const std::string &jsonOptions, bool persistOptions) override;
    ///Stops recording of frames
    /**
    Stops recording of triggered or free-run frames.
    @return true on success, false otherwise.
    */
    ...
    */
    ...
    ///Returns current recording options
    /**
    @return JSON string containing current recording options. The JSON options are described by schema
    RecordingOptionsSchema.json or RecordingOptionsSchemaStrict.json (with all fields as required).
    */
    virtual std::string RecordingOptions() override;
    ///Returns frame index of last recorded frame
    /**
    @return positive number: last recorded frame index,
        -1: No frame saved yet,
        -2: Failure (generic, timeout),
        -3: Device not connected
    */
    virtual int LastRecordedFrameIndex() override;
#endif
    ...
    std::unique_ptr<DepthMapAligner> Aligner;
    std::unique_ptr<PhoXiCommunicationTools> CommunicationTools;
#endif

#ifdef PHOXI_API_CLIENT
    std::unique_ptr<pho::api::ApiFrameConsumerIPC> ApiFrameConsumer;
#endif
  ...
  private:
    std::chrono::milliseconds lastSyncHWStatusTimestamp{-1};

    enum class PhoXiDeviceState {
        Connecting, Connected, Disconnecting, Disconnected
    };
#ifdef PHOXI_API_CLIENT
    PhoXiDeviceState _interprocStatus;
#endif
};
#pragma pack(pop)
...
struct PHOXI_DLL_API PhoXiDeviceInformation {
    ...
    std::string Variant;
    ///List of features separated by a semicolon
    ///Helper parser \sa CheckFeature
    std::string Features;
    ...
    ///Check if given feature is present
    bool CheckFeature(const std::string& what) const;
    operator std::string() const;
};
#pragma pack(pop)
...
class PHOXI_DLL_API PhoXiFactory {
  public:
    ...
    */
    ...
    @param Type - PhoXiDeviceType enumerator - device type.
    @param IP - IPv4 or IPv6 address of the device (Link-local and 
    scope-local IPv6 address must include scope ID).
    @return handle to a connected PhoXi device or nullptr on failure.
    */
    PPhoXi CreateAndConnect(const std::string &HWIdentification, PhoXiDeviceType Type, const std::string &IP);
    ...
    */
    ...
    ///Reboots device with specified HWIdentification
    /**
    Issues a reboot command to a device with specified HWIdentification
    if device is found and if device is !not a file camera which can not
    be rebooted.
    @param HWIdentification - The input Hardware identification string,
    or ID (same as in PhoXi Control) of the device.
    @return true when reboot command was successfully issued or false if not.
    */
    bool Reboot(const std::string& HWIdentification);
#endif
    ...
    @return true on success, false otherwise.
    */
    bool CanConnect(const std::string& HardwareIdentification);
  private:
    friend class PhoXi;
    friend class ClientSideApp;
    bool ConnectPhoXi(const std::string &PhoXiType, const std::string &HWIdentification, CommandConnectResponse &response);
    bool ConnectPhoXi(const std::string &PhoXiType, const std::string &HWIdentification, const std::string &IPv4, const std::string &Mode, CommandConnectResponse &response);
